...
function cameraErrorMessage(err: unknown): string {
  ...
  switch (name) {
    ...
    case "PermissionDeniedError":
      return "Permesso fotocamera negato. Attivalo per questo sito e premi Riprova.";
    case "NotFoundError":
...
export function BarcodeScanner({ onScan, paused = false, className }: BarcodeScannerProps) {
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      if (cancelled) return;
      if (perm === "denied") {
        setError("Permesso fotocamera negato. Attivalo nelle impostazioni e premi Riprova.");
        setManualOpen(true);
        return;
      }
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-2xl bg-black", className)}>
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
        <div className="relative h-44 w-[78%] max-w-sm rounded-lg border-2 border-white/80 shadow-[0_0_0_9999px_rgba(0,0,0,0.35)]">
          <div className="absolute left-0 right-0 top-1/2 h-0.5 -translate-y-1/2 animate-pulse bg-[var(--gold)]/80" />
        </div>
      </div>
      ...
      <div className="absolute bottom-3 left-3 right-3 flex flex-wrap items-center gap-2">
        ...
        <Button type="button" size="sm" variant="outline" onClick={() => setManualOpen((v) => !v)}>
